...
namespace Incerrance.WebApp.Controllers
{
    public class InsurranceController : Controller
    {
        ...
        public ActionResult LoadLastedInsurrance(Guid? id)
        {
            Insurrance insurrance = db.Insurrances.Find(id);
            List<Insurrance> list = new List<Insurrance>();
            list = db.Insurrances.Where(x => x.IsDeleted == false && x.InsurranceCategoryId == insurrance.InsurranceCategoryId && x.Id != insurrance.Id).OrderByDescending(x => x.CreatedOn).Take(2).ToList();
            ViewBag.Count = list.Count();
            return PartialView(list);
        }
        ...
        {
            List<InsurranceCategory> insurranceCategory = new List<InsurranceCategory>();
            insurranceCategory = db.InsurranceCategories.Where(x => x.IsDeleted == false).ToList();
        ...
        {
            List<Insurrance> list = new List<Insurrance>();
            list = db.Insurrances.Where(x => x.IsDeleted == false && x.InsurranceCategoryId == insurranceCategory.Id).ToList();
            ViewBag.Count = list.Count();
            return View(list);
        }
